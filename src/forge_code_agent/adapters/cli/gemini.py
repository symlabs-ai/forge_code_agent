...
class GeminiProviderAdapter(ProviderAdapter):
    """
    Adapter Gemini Code-like baseado em CLI.

    MVP: segue a mesma estrutura de Codex/Claude, com identificador próprio.
    """
    ...
    def _build_command(self, request: ExecutionRequest) -> list[str]:
        """
        Construir comando para execução do provider Gemini.

        Modos suportados:
        - modo "real": se FORGE_CODE_AGENT_GEMINI_CMD estiver definida, usa esse
          comando como base, adicionando o prompt como argumento. Exemplo:
            gemini --output-format json --yolo
          O workspace é controlado via cwd no subprocess (não via flag).
        - modo "simulado" (default em ambientes de teste/offline): script
          Python inline que gera conteúdo previsível.
        """
        base_cmd = os.getenv("FORGE_CODE_AGENT_GEMINI_CMD")
        if base_cmd:
    ...
    def run(self, request: ExecutionRequest) -> ExecutionResult:
        ...
        # Para o Gemini real, o diretório de trabalho é controlado via cwd;
        # no modo simulado, cwd pode ser None sem impacto.
        completed = self.executor.run(cmd, timeout=request.timeout, cwd=str(request.workdir) if request.workdir else None)
    ...
    def stream(self, request: ExecutionRequest) -> Iterable[dict]:
        ...
        - modo "real": se FORGE_CODE_AGENT_GEMINI_STREAM_CMD estiver definida,
          usa esse comando como base, adicionando o prompt como argumento.
          Exemplo:
            gemini --output-format stream-json --yolo
          Controla o workspace via cwd.
        - modo "simulado": script Python inline que emite o conteúdo em dois
          chunks.
        """
        base_cmd = os.getenv("FORGE_CODE_AGENT_GEMINI_STREAM_CMD")
        if base_cmd:
            base = shlex.split(base_cmd)
            try:
